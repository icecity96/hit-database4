...
// Entry represents the key-values pair contained within nodes
type Entry struct {
	Key   interface{}
...
func (tree *Tree) Put(key interface{}, value interface{}) {
	...
	valuemap[value] = true
	entry := &Entry{Key: key, Value: valuemap}
...
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree) Get(key interface{}) (value map[interface{}]bool, found bool) {
...
func (tree *Tree) insertIntoLeaf(node *Node, entry *Entry) (inserted bool) {
	...
	if found {
		for v := range entry.Value {
			node.Entries[insertPosition].Value[v] = true
		}
		return false
	}
